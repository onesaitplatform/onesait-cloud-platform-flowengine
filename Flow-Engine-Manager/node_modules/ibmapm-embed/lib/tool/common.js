...
exports.polishServiceName = function(providerId, namespace, servicename) {
    var tid = process.env.UA_LWDC_LISTENER_URL ? '_tenantid_' : process.env.APM_TENANT_ID;
    if (k8sutil.getUaK8Info()) {
        serviceId = k8sutil.getUaK8Info()['serviceID'];
        return k8sutil.getUaK8Info()['serviceName'];
    } else if (tid && providerId && servicename) {
