<!-- Onesait Platform Notebook Node-RED node HTML file -->
	<script type="text/x-red" data-template-name="Notebook Launcher">
		...
		<div class="form-row">
			...
			<br/><br/>
			<label for="node-input-topic" style="width: 130px;"><i class="fa fa-book"></i> Onesait Platform Notebook</label>
	...
	<script type="text/x-red" data-help-name="Notebook Launcher">
	   <span><img style="margin-left: 5px;" src="icons/platform_logo.png"/><h2 style="display:inline"> Onesait Platform Notebook Launcher</h2>
	   <p><code>Onesait Platform Notebook</code> Selection of user notebook.</p>
	   <p><code>Workflow mode</code> If activated, execute the node waiting for all Onesait Platform notebook inputs.</p>
	   <p><code>Timeout</code> Timeout in seconds of Onesait Platform notebook from node input paragraph to execution complete. If timeout is reach Onesait Platform notebook execution will be stopped and no output will be send</p>
	   <h3>Inputs & Execution</h3>
	   <p><code>Input Paragraph selection</code> Optional selection of input paragraph of Onesait Platform notebook. The input paragraph will be executed before the entire notebook to set its parameters</p>
	   ...
	   <p><code>Outputs Number</code> Number of paragraph outputs that the Onesait Platform notebook execution will have. </p>
	   ...
	   <p><code>Output Parameters mapping</code> List of Onesait Platform notebook outputs (depending directly of Outputs number), each output will be determinated for a paragraph and a topic. </p>
	...
	<script type="text/javascript">
		...
		RED.nodes.registerType('Notebook Launcher',{
			...
			outputLabels: function(i) {
				return JSON.parse(this.outputparameters)[i].paragraphDesc;
				//return this.id+" output #"+i;
			},
			...
			oneditprepare: function() {
				...
				//Properties from OnesaitPlatform config
				var scriptBasePath = onesaitPlatformConfig.scriptBasePath;
				//Parameters to config
				var onesaitPlatformNotebookUrlService=scriptBasePath + '/script/services/api_motorflujo/v01/notebooks'
				var onesaitPlatformNotebookUrlServiceInfo=scriptBasePath + '/script/services/api_motorflujo/v01/notebookInfo'
				...
				$('#node-input-fullexecutionnotebook').change(
					function(){
						if ($("#node-input-fullexecutionnotebook").prop("checked") === true){
							//Enable outputs config
							$('#node-input-enableoutputs').removeAttr("disabled");
							$('#node-input-enableoutputs').prop('checked', true);
							
							$("#outputForm").show();
							//Disable inputs
							$("#nodeParametersForm").hide();
							...
							//disable outputs config. Output must just be the paragraph reuslt

							$("#node-input-splitoutputs").prop('checked', false);
							$('#node-input-enableoutputs').prop('checked', false);
							$('#node-input-enableoutputs').attr("disabled", true);
							$("#outputForm").hide();
							//Enable inputs
							$("#nodeParametersForm").show();
							$("#node-input-outputs").val(1);
				...
				$('#parametersMapping').on('DOMSubtreeModified propertychange', function() {
					setTimeout(function() { $(".red-ui-typedInput-container").css("width","60%");},0);
				});
				...
				function getNotebookJSONData(notebookIDParam){
					...
					var jsonCallbackName = 'NotebookInfo';
			        var urlService = onesaitPlatformConfig.scriptBasePath + '/flowengine/node/services/user/notebooks/paragraphs?notebookZeId='+notebookIDParam+'&authentication=';
					...
					$.ajax({
						...
						success: function(response, textStatus, jqXHR) {
							...
							if(textStatus == "success"){
								...
								notebookJSON=JSON.parse(response);
				...
				function reloadInputForm(){
					...
					notebookJSON.paragraphs.filter(function(paragraph){return paragraph.id==paragraphID}).map(function(paragraph){
						...
						for(var field in paragraph.settings.forms){
							...
							$('#parametersMapping').append('<div class="form-row"><label style="width=30%; margin-right:10px" for="node-input-' + field + '"><i class="fa fa-sign-in"></i> ' + field + '</label><input type="text" style="width=60%;" class="node-input" id="node-input-' + field + '" placeholder=""><input type="hidden" class="node-input" id="node-input-' + field + '_type" placeholder="" value="' + typeTemp + '"></div>');
							...
							$("#node-input-" + field).typedInput('width','60%');
				...
				function reloadOutputForm(){
					...
					for(var output=0;output<$("#node-input-outputsnosplit").val()&&notebookJSON;output++){
						...
						$('#parametersOutputMapping').append('<div class="form-row"><label for="node-input-output-' + output + '"><i class="fa fa-sign-out"></i> Output ' + output + '</label><select class="node-output" id="node-input-output-' + output + '" placeholder="" value="' + paragraphTemp + '"/></div>');
						for(var paragraph=0;paragraph<notebookJSON.paragraphs.length;paragraph++){
				...
				function getNotebooks(){
			        ...
			        var urlService = onesaitPlatformConfig.scriptBasePath + '/flowengine/node/services/user/notebooks?authentication=';
			        var url = urlService + auth + '&callback=' +jsonCallbackName;
					$.ajax({
						url: url, 
						dataType: "jsonp",
						...
						success: function(data, textStatus, jqXHR) {
							if(textStatus == "success"){
								var notebooks = data;
								for(var notebook=0;notebook<notebooks.length;notebook++){
									$('#node-input-notebook').append($('<option>',
										{
											value: notebooks[notebook].id,
											text: notebooks[notebook].name,
											title: notebooks[notebook].name
										})
									);
								}
								...
								console.log("Unable to load user Notebooks.");
							}
						}
						,
						error: function(xhr, status, error) {
			...
			oneditsave: function() {
				...
				var jsonParamOutput = [];
				$(".node-output option:selected").each(
					function(){
						//jsonParamOutput.push({paragraph:this.value,topic:$("#"+this.id+"_topic").val()});
						jsonParamOutput.push({paragraph:this.value, paragraphDesc:this.text});
					}
				)
				if (!$("#node-input-enableoutputs").prop("disabled"))  {
					outputs = parseInt($("#node-input-outputs").val());
				} else {
					outputs = 1;
				}
