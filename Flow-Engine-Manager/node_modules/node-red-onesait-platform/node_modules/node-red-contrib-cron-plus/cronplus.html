<script type="text/javascript">
    RED.nodes.registerType('cronplus', {
        ...
        oneditprepare: function () {
            debugger
            let $tipContainer = $("#node-input-crontab-tip-container");
            $tipContainer.hide();
            ...
            var updateCronDescription = function (target) {
                updateCronDescription.target = target ? target : updateCronDescription.target;
                let $tipContainer = $("#node-input-crontab-tip-container");
                let $tip = $("#node-input-crontab-tip");
                let $tipTitle = $("#node-input-crontab-tip-title");
                try {
                    let $expression = $(updateCronDescription.target);
                    ...
                    $tip.text("Getting description for " + e.expression + "...").show();
                    ...
                    .done(function (data) {
                        $tipTitle.html('Description of <span class="cron-expression">' + e.expression + '</span> ...');
            ...
            function generateOption(i, option) {
                var $tip = $("#node-input-crontab-tip-container");
                ...
                var payloadField = $('<input/>',{class:"node-input-option-payload",type:"text",style:"margin-left:7px; width:calc(37% - 16px);", placeholder: 'payload', value:option.payload}).appendTo(div).typedInput({default:option.type||'str',types:['flow', 'global', 'str', 'num', 'bool', 'json', 'bin', 'date', 'env']});
                ...
                debugger
                expressionField.on('focus keyup paste cut',function() {
                    $tip.show();
                    updateCronDescription(this);
                });                
                expressionField.on('blur',function() {
                    $tip.hide();
                });
            ...
            $("#node-input-crontab").change(function () {
                updateCronDescription("#node-input-crontab");
            });
...
<script type="text/x-red" data-template-name="cronplus">
    ...
    <div class="form-row node-input-option-container-row" style="margin-bottom: 0px;width: 100%">
        ...
        <div id="node-input-option-container-div" style="box-sizing: border-box; border-radius: 5px; height: 150px; padding: 5px; border: 1px solid #ccc; overflow-y:scroll;display: inline-block; width: calc(70% + 15px);">
    ...
    <div id="node-input-crontab-tip-container">
        <hr>
        <div class="form-row">
            <p style="font-size: 14px; font-weight: bold" id="node-input-crontab-tip-title"></p>
