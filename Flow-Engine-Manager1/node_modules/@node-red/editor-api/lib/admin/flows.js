...
//Begin Onesait Platform custom code
var onesaitPlatform = require("../../onesait-platform/onesait-platform-comm.js");
//End Onesait Platform custom code
...
module.exports = {
    ...
    post: function(req,res) {
        ...
        var flowConfig = req.body;
        
        runtimeAPI.flows.setFlows(opts).then(function(result) {
            if (version === "v1") {
                onesaitPlatform.notifyNodes(flowConfig, res, "v1", 204);
                //res.status(204).end();
            } else {
                flowConfig = flowConfig.flows;
                onesaitPlatform.notifyNodes(flowConfig, res, "v2",result);
                //res.json(result);
            }
        }).catch(function(err) {
            apiUtils.rejectHandler(req,res,err);
